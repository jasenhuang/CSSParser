...
stylesheet
    : ws ( charset ( Comment | Space | Cdo | Cdc )* )* ( imports ( Comment | Space | Cdo | Cdc )* )* ( g_namespace ( Comment | Space | Cdo | Cdc )* )* ( statement ( Comment | Space | Cdo | Cdc )* )*
    ;
...
block
    : '{' ws (  declarationList | statement | any | block | atKeyword ws | ';' ws )* '}' ws
    ;
